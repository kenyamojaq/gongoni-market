(() => {
  ...
  function searchAlias(value){return /sufuria/i.test(value)?value.replace(/sufuria/ig,'cooking pot'):value;}
  function applySearch(value,shouldScroll=true){const realSearch=document.querySelector('#searchInput'),headerSearch=document.querySelector('#headerSearchInput');if(!realSearch)return;const displayValue=value;realSearch.value=searchAlias(value);if(headerSearch&&headerSearch.value!==displayValue)headerSearch.value=displayValue;realSearch.dispatchEvent(new Event('input',{bubbles:true}));if(headerSearch)headerSearch.value=displayValue;if(shouldScroll)document.querySelector('#products')?.scrollIntoView({behavior:'smooth',block:'start'});}
  function enableSufuriaAlias(){const realSearch=document.querySelector('#searchInput'),headerSearch=document.querySelector('#headerSearchInput');if(!realSearch||realSearch.dataset.sufuriaAlias==='1')return;realSearch.dataset.sufuriaAlias='1';realSearch.addEventListener('input',()=>{const typed=realSearch.value;if(!/sufuria/i.test(typed))return;realSearch.value=searchAlias(typed);setTimeout(()=>{realSearch.value=typed;if(headerSearch)headerSearch.value=typed;},0);},true);}
